...
with one crosspoint.
"""
function getDefaultCrossoverOp(individualType::DataType)::Tuple{Function, Array}
    method = nothing
    varArgs = []
    if individualType <: GAGenotype
        method = uniformCross
    elseif individualType <: CGPGenotype || individualType <: STGPGenotype
        method = subtreeCross
    elseif individualType <: GEPGenotype
        method = kPointRecombinationCross
    end
...
function cross_(crossoverOp::CrossoverOperator, selected::Array{Individual},
    ...
    if experimentInfo._individualType <: GPGenotype
        method = (selectedParents) -> crossoverOp._method(selectedParents..., experimentInfo._GPExperimentInfo, rng, crossoverOp._varArgs...)
    else
        method = (selectedParents) -> crossoverOp._method(selectedParents..., rng, crossoverOp._varArgs...)
    end
