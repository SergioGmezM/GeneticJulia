    ...
    while i != 0
        ...
        if typeof(representation[i]) <: FunctionNode
            ...
            if findfirst(x->x==i, visitedNodes) == nothing
                push!(visitedNodes, i)
                push!(phenotype, getFunctionName(representation[i]) * "(")
            end
            ...
            if typeof(representation[i]) <: VariableNode
                push!(phenotype, getVariableName(representation[i]) * ",")

            elseif typeof(representation[i]) <: ConstantNode
                push!(phenotype, string(eval(representation[i])) * ",")

            elseif typeof(representation[i]) <: NoArgsFunctionNode
                push!(phenotype, getNoArgsFunctionName(representation[i]) * "(),")
            end
